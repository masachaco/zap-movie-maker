...
class Clip:
    ...
    @classmethod
    def create_text_clip(cls,clips, text_clip_options, current_duration, has_audio,skip_audio_render=False):
        """
        テロップを作成する
        """
    ...
    @classmethod
    def set_mark(cls, name="default") -> None:
        Clip.scripts.append({
            "command": "mark",
            "name": name
        })
    ...
    @classmethod
    def composit_movie(cls,skip_audio_render=False):
        ...
        for script in Clip.scripts:
            ...
            if command == "char":
                ...
                pass
            if command == "voice":
                txtclip = Clip.create_text_clip(Clip.movie["text_clips"], script, Clip.movie["current_duration"], True,skip_audio_render=skip_audio_render)
                Clip.movie["current_duration"] += txtclip.duration
                Clip.movie["text_clips"].append(txtclip)
                pass
            if command == "text":
                txtclip = Clip.create_text_clip(Clip.movie["text_clips"], script, Clip.movie["current_duration"], False,skip_audio_render=skip_audio_render)
                Clip.movie["current_duration"] += txtclip.duration
                Clip.movie["text_clips"].append(txtclip)
                pass
            if command == "bgm":
            ...
            if command == "wait":
                ...
                pass
            if command == "mark":
                Clip.mark[script["name"]] = Clip.movie["current_duration"]
                print("mark",Clip.mark)
                pass
    ...
    def preview(cls,audio_samp_rate=11000,preview_fps=5,preview_resize=1,preview_range=None, preview_range_by_mark=None,skip_audio_render=False):
        final = Clip.composit_movie(skip_audio_render=skip_audio_render)
        if preview_range is not None:
            final = final.subclip(preview_range[0], preview_range[1])
        if preview_range_by_mark is not None:
            start = Clip.mark[preview_range_by_mark[0]]
            end = Clip.mark[preview_range_by_mark[1]]
